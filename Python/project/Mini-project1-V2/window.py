...
class Application(tk.Tk):
    ...
    def add_selectFrame(self):
        ...
        tk.Button(self.selectFrame,text="valider", command=self.validateSelect).grid(row=2, column=0)
    ...
    def validateSelect(self):
        ...
        for n in range(3):
            ...
            tk.Button(self.selectFrame, text=str(self.gateway.getProductFacts(productN, option=("name"))), command=partial(self.chooseProduct, n)).grid(row=n, column=1)

    def chooseProduct(self, n):
        self.productSelected = self.gateway.getProductInDump(dump=self.dump, number=n)
    ...
    def switch_infoFrame(self):
        ...
        buffvar:str = "fiche info de :" + str(self.gateway.getProductFacts(self.productSelected, ("name"))["name"])
        #productName = tk.StringVar().set(buffvar)
        tk.Label(self.infoFrame, text=buffvar).grid(row=0, column=0)

        optionInside = ("name", "ingredients", "code", "ecoscore_score", "ecoscore_grade","nutriscore_grade", "stores", "packaging", "quantity", "brands", "labels")
        for i in range(len(optionInside)):
            try:
                msgBuff:str = str(optionInside[i]) + " --> " +  self.gateway.getProductFacts(self.productSelected, option=(optionInside[i]))[optionInside[i]]
            except:
                msgBuff:str = str(optionInside[i]) + " --> " + "information inconnu"
